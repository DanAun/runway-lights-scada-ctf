...
SOLVE_DELAY = 5 # Number of seconds light needs to be on before considering the challenge solved
...
# Dummy user data for authentication with hashed passwords
...
is_solved = False # Whether the team has already solved the challenge
def is_challenge_solved():
    ...
    global is_solved
    if is_solved:
        log.debug("Blocked attempt to turn on lights for team %d since challenge is marked as already solved!" % team_num)
        return False
...
def monitor_and_control():
    ...
    while True:
        ...
        if current_state != previous_state:
            ...
            if team_num is not None and is_challenge_solved():
                activate_team_light(team_num)
